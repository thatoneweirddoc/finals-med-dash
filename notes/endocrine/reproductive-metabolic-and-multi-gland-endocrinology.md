---
title: "Reproductive, Metabolic & Multi-Gland Endocrinology"
system: "Endocrine"
sources: [Adrenal Summary — Adrenocortical carcinoma, Endocrine Master Notes, Endocrinology (ANU) — Thyroglossaal cyst, Endocrinology (ANU) — metabolic syndrome, Endocrinology (ANU) — tumours of pancreas]
tags: [Reproductive, MEN, PCOS, RET]
updated: "2026-08-14"
---

## Reproductive, Metabolic & Multi-Gland Endocrinology


### Endocrine Master Notes — Reproductive, Metabolic & Multi-Gland Endocrinology

#### 10.1 Female Reproductive Endocrinology

##### Polycystic ovary syndrome (PCOS)
Commonest endocrine disorder in women of reproductive age; a state of **hyperandrogenism + insulin resistance**.
- **Rotterdam criteria** — diagnosis needs **2 of 3**: (1) **oligo/anovulation**, (2) **clinical or biochemical hyperandrogenism** (hirsutism, acne; raised free testosterone), (3) **polycystic ovaries on ultrasound** — after excluding mimics (CAH, Cushing's, androgen-secreting tumour, hyperprolactinaemia, thyroid disease). ⚑
- **Associated:** obesity, insulin resistance/impaired glucose tolerance/T2DM, dyslipidaemia, subfertility, **endometrial hyperplasia** risk (from unopposed oestrogen).
- **Biochemistry clue:** raised **LH:FSH ratio** (classic but not required), raised free testosterone with **normal/low SHBG**, normal/mildly raised androgens (a *very* high testosterone suggests a tumour instead).
- **Management:** weight loss/lifestyle first; **combined oral contraceptive** for menstrual regulation/hyperandrogenism; **metformin** for metabolic features; **letrozole** (first-line) or clomifene for ovulation induction; ensure endometrial protection (regular withdrawal bleeds).

##### Amenorrhoea and hirsutism
- **Primary amenorrhoea** (no menarche): Turner's, constitutional delay, imperforate hymen, androgen insensitivity, GnRH deficiency (Kallmann's — with anosmia).
- **Secondary amenorrhoea:** **pregnancy first**, then hypothalamic (low weight/stress/exercise), **hyperprolactinaemia**, PCOS, thyroid disease, premature ovarian insufficiency, Sheehan's, Asherman's.
- **Rapid virilisation** (deep voice, clitoromegaly, rapid hirsutism) → **androgen-secreting tumour or CAH**, not PCOS.

##### Menopause
Cessation of ovarian function → oestrogen deficiency: vasomotor symptoms, urogenital atrophy, mood/sleep change, accelerated bone loss. **HRT** relieves symptoms (weigh VTE/breast-cancer risk; give a progestogen with oestrogen if the uterus is present).

#### 10.2 Male Hypogonadism and Gynaecomastia

| | Primary (hypergonadotrophic) | Secondary (hypogonadotrophic) |
|---|---|---|
| Problem | Testis | Pituitary/hypothalamus |
| Testosterone | Low | Low |
| **LH/FSH** | **High** | **Low/normal** |
| Causes | **Klinefelter's (47,XXY)**, mumps orchitis, trauma, chemo | Hypopituitarism, **Kallmann's** (+anosmia), hyperprolactinaemia, obesity, opioids, steroids |

**Gynaecomastia** = benign glandular breast tissue in a male from a **raised oestrogen:androgen ratio**: physiological (neonatal, puberty, elderly), drugs (**spironolactone, digoxin, cimetidine, antipsychotics, cannabis, anti-androgens**), liver disease, hyperthyroidism, hypogonadism, and — important not to miss — **testicular/hCG-secreting tumours**. Distinguish from fatty pseudogynaecomastia and exclude malignancy if unilateral/hard/fixed.

#### 10.3 Multiple Endocrine Neoplasia and Neuroendocrine Tumours

**MEN syndromes** — autosomal dominant; learn the components (very high yield):

| Syndrome | Gene | Components |
|---|---|---|
| **MEN 1** ("**3 Ps**") | **MEN1** (menin) | **Parathyroid** hyperplasia (commonest/first), **Pancreatic** islet tumours (gastrinoma/insulinoma), **Pituitary** adenoma |
| **MEN 2A** | **RET** | **Medullary thyroid carcinoma**, **Phaeochromocytoma**, **Parathyroid** hyperplasia |
| **MEN 2B** | **RET** | Medullary thyroid carcinoma, phaeochromocytoma, **mucosal neuromas + marfanoid habitus** (no parathyroid disease) |

*Memory: MEN1 = "3 Ps" (Parathyroid, Pancreas, Pituitary); MEN2A = medullary + phaeo + parathyroid; MEN2B = medullary + phaeo + **mucosal neuromas/marfanoid** (Ps become "2 Ms").* **All MEN2 subtypes → medullary thyroid carcinoma** — RET-positive relatives are offered **prophylactic thyroidectomy**; always **exclude phaeochromocytoma before any surgery**.

**Carcinoid / neuroendocrine tumours:**
- **Carcinoid syndrome** (flushing, diarrhoea, wheeze, right-sided valvular heart disease) occurs when a serotonin-secreting NET **metastasises to the liver** (bypassing first-pass metabolism) or is bronchial. Diagnose with **urinary 5-HIAA** and chromogranin A; localise with somatostatin-receptor imaging (Ga-DOTATATE). Treat with **somatostatin analogues (octreotide)**; **carcinoid crisis** is precipitated by handling/anaesthesia → octreotide cover.
- Other functional pancreatic NETs: **insulinoma** (hypoglycaemia), **gastrinoma** (Zollinger–Ellison → refractory peptic ulcers/diarrhoea), **VIPoma** (watery diarrhoea), **glucagonoma** (necrolytic migratory erythema, diabetes).

#### 10.4 Obesity and Metabolic Syndrome

- **Metabolic syndrome:** clustering of **central obesity, hypertension, dyslipidaemia (high triglycerides/low HDL), and dysglycaemia/insulin resistance** — a cardiovascular and diabetes risk state.
- **Obesity management:** lifestyle first; **incretin-based pharmacotherapy** (GLP-1 RA **semaglutide**, GIP/GLP-1 **tirzepatide**) now has a major role and substantial weight-loss efficacy ⚑; **bariatric surgery** for higher BMI thresholds (typically BMI ≥40, or ≥35 with comorbidity; lower thresholds considered in certain groups) with durable weight loss and diabetes remission.

---

#### Rapid-Recall Discriminators — Section 10

| Clue / question | Answer |
|---|---|
| PCOS diagnostic framework | **Rotterdam — 2 of 3** (oligo-ovulation, hyperandrogenism, polycystic ovaries) |
| First-line ovulation induction in PCOS | **Letrozole** |
| Very high testosterone + rapid virilisation | **Androgen-secreting tumour** (not PCOS) |
| First test in secondary amenorrhoea | **Pregnancy test** |
| Hypogonadism + anosmia | **Kallmann syndrome** |
| Male hypogonadism with **high** LH/FSH | **Primary** (e.g. Klinefelter's 47,XXY) |
| Drug causes of gynaecomastia | Spironolactone, digoxin, cimetidine, antipsychotics, cannabis |
| MEN 1 components | **Parathyroid, Pancreas, Pituitary (3 Ps)** |
| MEN 2A components | Medullary thyroid ca, phaeochromocytoma, parathyroid |
| MEN 2B distinguishing feature | **Mucosal neuromas + marfanoid** (no parathyroid) |
| Gene in MEN 2 | **RET** |
| Always exclude before MEN2 surgery | **Phaeochromocytoma** |
| Carcinoid syndrome needs | **Liver metastases** (or bronchial primary) |
| Test for carcinoid | **Urinary 5-HIAA** (+ chromogranin A) |
| Refractory peptic ulcers + diarrhoea | **Gastrinoma (Zollinger–Ellison)** |

##### Sources
Endocrine Society/relevant guidelines (PCOS Rotterdam criteria, MEN, neuroendocrine tumours); NICE CKS (amenorrhoea, menopause, obesity); Merck Manual Professional; BNF (drug causes of gynaecomastia). Incretin obesity pharmacotherapy flagged ⚑ and cross-checked against current NICE/guideline positions.

---


---


### Endocrinology (ANU) — tumours of pancreas — Reproductive, Metabolic & Multi-Gland Endocrinology

| GLUCAGONOMA | Tumour of pancreatic α cells → overproduction of glucagonPresentation: dermatitis (necrolytic migratory erythema), diabetes (hyperglycaemia), DVT, declining weight, depressionTreatment: octreotide, surgery |
| --- | --- |
| INSULINOMA | Tumour of pancreatic β cells → overproduction of insulin → hypoglycaemiaPresentation: Whipple’s triad, ↑ C-peptide levels∼ 10% of cases associated with MEN 1 syndromeTreatment: surgical resection |
| SOMATOSTATINOMA | Tumour of pancreatic δ cells → overproduction of somatostatin → ↓ secretion of secretin,cholecystokinin, glucagon, insulin, gastrin, gastric inhibitory peptide (GIP)Presentation: diabetes/glucose intolerance, steatorrhea, gallstones, achlorhydriaTreatment: surgical resection, somatostatin analogs (e.g., octreotide) for symptom control |


---


### Endocrinology (ANU) — metabolic syndrome — Reproductive, Metabolic & Multi-Gland Endocrinology

| DEFINITION | Also known as insulin resistance syndrome or Syndrome XSeveral risk factors that increase the risk of cardiovascular disease, type 2 diabetes, and fatty liverPresence of ≥ 3 of the following conditions (or already receiving medical treatment for them) Insulin resistance: fasting glucose ≥ 100 mg/dLElevated blood pressure: ≥ 130/85 mm HgElevated triglycerides: ≥ 150 mg/dL Low HDL-C: in men &lt; 40 mg/dL; in women &lt; 50 mg/dLAbdominal obesity: waist circumference ≥ 102 cm in men; ≥ 88 cm in women |
| --- | --- |
| MANAGEMENT | First-line: lifestyle modificationsDietary changes: calorie restriction, healthy foods (e.g., fruit/vegetables, protein-rich, unsaturated fats, sodium-restricted)Physical activity: minimum of 30 minutes moderate exercise per day (2.5 hours per week) , which increases insulin sensitivity, lowers blood pressure, and promotes weight lossMedical therapy: treat hypertension, diabetes mellitus, and dyslipidaemiaBariatric surgery: if BMI ≥ 40 and no success with dietary and lifestyle changes Sleeve gastrectomy: large part of the greater curvature is removed, so that the remaining stomach resembles a sleeveRoux-en-Y gastric bypass |

thyroid disorders


---


### Endocrinology (ANU) — Thyroglossaal cyst — Reproductive, Metabolic & Multi-Gland Endocrinology

| DEFINITION | A fibrous cyst that is a remnant of the thyroglossal duct that forms during the embryonic development of the thyroid gland |
| --- | --- |
| EPIDEMIOLOGY | Most common midline neck mass in childrenMost common before the age of 20 |
| PATHOGENESIS | The thyroid gland originates from the foramen cecum at the base of the tongue and descends caudally into the neck, creating the thyroglossal duct. If the duct fails to obliterate, midline neck cysts or ectopic thyroid tissue can develop anywhere along the path of the thyroglossal duct. These cysts can be filled with fluid or mucous, and may enlarge if they become infected. Most lesions are infrahyoid |
| PRESENTATION | Painless, firm midline mass that is usually near the hyoid bone (can be painful if infected)Lump: feels smooth, moves up with protrusion of tongueDyspnoeaDysphagiaDyspepsia |
| INVESTIGATIONS | Neck and thyroid examinationUltrasound: evaluate cyst and confirm location of thyroidScreen TSH levels: may contain ectopic thyroid tissue which produce hormonesFine needle biopsy: If an infection is suspected, for Gram stain and culture (including AFB and mycobacterial culture) |
| COMPLICATIONS | Infection of the cyst and abscess formation, sinus tract extension to the skin, possible ectopic thyroid tissue, possible malignancy arising from ectopic thyroid tissue |
| MANAGEMENT | Sistrunk operation: cystectomy and excision of central hyoid bone to minimise recurrenceTotal thyroidectomy is controversial, but is advised in older patients with large tumours |


---


### Adrenal Summary — Adrenocortical carcinoma — Reproductive, Metabolic & Multi-Gland Endocrinology

**Aetiology - **sporadic or associated with genetic conditions like Li Fraumeni Syndrome with p53 mutations 🡪 lots of kinds of cancer eg breast, brain, acute leukemia

**Epidemiology - **extremely rare - 1-2 people per million.

- Macroscopic: &gt;100g (&gt;750g) yellowish cut surface. necrosis, haemorrhage,calcification.

**Microscopic**: solid, lobules Variable cortical cells, ↑ mitoses, necrosis, haemorrhage. +/- invasion into capsule or blood vessels

**Pathophysiology - **most common to least common hormone secreted in excess - cortisol,  androgens (virulising tumours) then rarely aldosterone (Conn’s syndrome)

**Prognosis - **usually aggressive and can (metastasize) rapidly because adrenal glands are so vascular. Because the adrenal gland resides in the retroperitoneum (surrounded in fat and residing behind the intestines and other abdominal organs), it can grow to a very large size before causing any sort of local compressive symptoms.

- Virilizing and Feminizing Adrenal Tumors
- rare and some are associated with adrenocortical carcinomas.
- A virilizing adrenal tumor makes excess androgens (testosterone). increased hair growth (hirsuitism), increased muscle mass, acne and amenorrhea (loss of periods in a female).

- A feminizing adrenal tumor makes excess oestrogen. increased growth of breast tissue (gynecomastia in men) and impotence in men.
